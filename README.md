...
| `persist_session` | `subagents.json` `rememberAgents` (default `true`) | Persist this subagent as a normal pi session instead of keeping the session in memory only; overrides the `rememberAgents` project default in both directions. The file goes in the `.subagents/` subdirectory of the project's session directory, out of the way of pi's `/resume`, `--continue` and prompt history — see [`rememberAgents`](#persistent-settings). It records its spawning session as parent. The subagent's `.output` transcript is still written either way unless `output_transcript: false` |
...
| `session_dir` | pi default | Session directory when `persist_session: true`; relative paths resolve from the agent cwd. Omitted, the session goes to the `.subagents/` subdirectory of pi's normal session location. Set, it goes exactly where you say and is **not** nested — naming a directory is already a deliberate placement, so it is honoured verbatim |
...
**Remember agents** (`rememberAgents`, default `true`): whether subagents persist their pi session, which is what lets [`@handle`](#agent-mentions) reopen an agent's conversation after its in-memory record has been evicted. So top-level subagents write a session file; agents spawned by another agent are excluded, since they get no handle and nothing could reopen their transcript. A custom agent's `persist_session` frontmatter overrides this per agent, in both directions. Toggle via `/agents → Settings → Remember agents`; with it off, handles expire with their record (roughly ten minutes past completion) and `@explore` then starts a fresh agent rather than resuming — the behaviour before this setting existed.

**Where those files go, and why it is not beside yours.** A subagent session is written to a `.subagents/` subdirectory of the session directory pi would otherwise have used — normally `~/.pi/agent/sessions/--<encoded cwd>--/.subagents/`. pi cannot tell a machine-driven session from one you started, so files sitting in the project directory itself reach four surfaces built for the latter: project-scope prompt history (a delegation prompt, often several KB, an arrow-up away), `pi --continue` (a subagent finishing after its spawner is the newest file by mtime, so it wins), `pi --session <id>`, and `/resume`, which nested them as child rows under their spawner. All four read one directory level and keep only `.jsonl` files, so one level of nesting is enough. This is unconditional and has no setting; an agent's [`session_dir`](#frontmatter-fields) still places its sessions verbatim.

Nothing about the files themselves changes — they are complete pi sessions, and `pi --session <path/to/file.jsonl>` opens one as it always did. What you give up is finding one by id: `pi --session <id>` resolves ids through the directory listings that no longer include them, and reports no match. `@handle` resume is unaffected, because it reopens by path.

Sessions written before this existed stay where they are. To relocate them, run `scripts/unnest-subagent-sessions.mjs` from a checkout of this repo. It is a dry run unless you pass `--apply`, and it moves a file only when it can identify one positively from its own header and name — a `parentSession`, and a session name of the form `<agentType>#<8 hex>`. Either signal alone is not enough: pi's `/branch` and `--fork` sessions carry a parent too. It is a rename within the same project directory, so anything it gets wrong is undone by moving the file back.

It refuses to move anything while a pi process is running, because a subagent mid-run keeps appending to its session file by path and would carry on writing to a new, headerless one at the old path. Quit pi first. `--force` overrides that check, and a per-file ten-minute age floor is kept in either case — but the floor is only a backstop, since an agent sitting inside one long tool call has a stale mtime and passes it.
...
| `subagents:agent-ended` | Every terminal transition (v3, alongside the completed/failed broadcasts) | `agentId`, `status` (native terminal status: `completed` \| `steered` \| `error` \| `aborted` \| `stopped`), `result?`, `error?` — for an **RPC** spawn, emitted only after that spawn's reply; other spawns emit as soon as they settle |
...
Other pi extensions can spawn, stop, and join subagents programmatically via the `pi.events` event bus, without importing this package directly.

All RPC replies use a standardized envelope: `{ success: true, data?: T }` on success, `{ success: false, error: string }` on failure.

**Full reference:** [`docs/rpc.md`](https://github.com/tintinweb/pi-subagents/blob/master/docs/rpc.md) — the complete spawn-option surface (including fields that are silently stripped), every error string, the completion-notification race, the `Symbol.for("pi-subagents:manager")` registry, and each protocol version's guarantees. [`tintinweb/pi-tasks`](https://github.com/tintinweb/pi-tasks) is the reference implementation.
...
```typescript
pi.events.on("subagents:ready", ({ sessionId }) => {
  // RPC handlers are registered; sessionId identifies the bound session.
});
```
...
scripts/              # One-shot local maintenance, run by hand; not published to npm
  unnest-subagent-sessions.mjs  # Move pre-.subagents/ session files into .subagents/
