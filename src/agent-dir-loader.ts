/**
 * agent-dir-loader.ts — Shared substrate: load AgentConfig maps from a single
 * directory of `*.md` agent files. The same parser, warnings, and reserved-name
 * rules back user agents (`custom-agents.ts`), skill-bundled agents
 * (`skill-agents.ts`), and package-registered agents (later).
 */
 ...
 * sorted before parsing so the load order (and thus the last-wins outcome of a
 * declared-name clash) is deterministic across platforms.
 */
export function loadAgentsFromDirectory(
  ...
  loadAgentsFromDirectoryInto(agents, dir, source, strict);
  return agents;
}

/** Load agent configs from a directory, merging into an existing map (last file wins). */
export function loadAgentsFromDirectoryInto(
...
/**
 * Close a load cycle: promote this cycle's warnings so an unchanged problem is
 * suppressed next time, and start a fresh set. Called once per `loadCustomAgents`.
 */
export function resetLoadWarnings(): void {
