 ...
 * completion-notification race, and the guarantees attached to each protocol version.
 */

import { isTopLevelAgent } from "./agent-manager.js";
import { type ModelRegistry, resolveModel } from "./model-resolver.js";
import { checkModelScope } from "./model-scope.js";
import type { AgentRecord, AgentTerminalStatus } from "./types.js";
...
export function registerRpcHandlers(deps: RpcDeps): RpcHandle {
  ...
  const pendingSpawnIds = new Map<string, string>();
  ...
  const unsubSpawn = handleRpc<{ requestId: string; type: string; prompt: string; options?: any }>(
    events, "subagents:rpc:spawn", async ({ requestId, type, prompt, options }) => {
      ...
      pendingSpawnIds.set(requestId, id);
      // Mark synchronously (before awaiting startup) so an immediately-failing
      // run's `agent-ended` event remains buffered until the RPC reply.
      agentEnded?.markSpawnPending(id);
      // With isolation: "worktree" the agent starts asynchronously. Waiting
      // keeps a strict-isolation failure in the RPC error envelope.
      await manager.awaitStartup(id);
      return { id };
    },
    ({ requestId }) => {
      const id = pendingSpawnIds.get(requestId);
      pendingSpawnIds.delete(requestId);
