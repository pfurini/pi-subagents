...
describe("cross-extension RPC", () => {
  ...
  describe("spawn RPC agent-ended ordering", () => {
    ...
    it("flushes a terminal event after a startup-error reply", async () => {
      const order: string[] = [];
      const gate = createAgentEndedGate(() => order.push("agent-ended"));
      (manager.spawn as ReturnType<typeof vi.fn>).mockImplementation(() => {
        queueMicrotask(() => gate.emit({ agentId: "agent-42", status: "error" }));
      ...
      (manager.awaitStartup as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("startup failed"));
      registerRpcHandlers({ ...deps, agentEnded: gate });

      events.on("subagents:rpc:spawn:reply:req-o3", (reply) => {
        order.push((reply as { success: boolean }).success ? "success-reply" : "error-reply");
      });
      events.emit("subagents:rpc:spawn", { requestId: "req-o3", type: "general-purpose", prompt: "x" });

      await vi.waitFor(() => expect(order).toContain("agent-ended"));
      expect(order).toEqual(["error-reply", "agent-ended"]);
    });
  });
